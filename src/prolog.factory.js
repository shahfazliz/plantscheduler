/* 
    Original interpreter is from http://ioctl.org/logic/prolog-latest
    Edited to make it compatible with angular js
*/

/*global angular */
(function () {
    angular.module('PrologInterpreter', []);
    
    function PrologFactory() {
        var outr = [],
            output = "";
        ...
        function freeform(query) {
        	cls();
        
            // Parsing query
        	query = ParseBody(new Tokeniser(query));
        	
        	// Error occurred parsing the query
        	if (query == null) {
        		throw new Error('An error occurred parsing the query');
        	}
        	
        	query = new Body(query);
        	
        	var vs = varNames(query.list);
        
        	// Prove the query.
        	prove(renameVariables(query.list, 0, []), [], outr, 1, applyOne(printVars, vs));
    ...
    PrologFactory.$inject = [];
    
    angular.module('PrologInterpreter').factory('PrologFactory', PrologFactory);
}());